...
import { MediaImageWrapper } from './ChatStyledComponents';
...
export const MediaDisplay = ({ msg, openLightbox }: { msg: Message, openLightbox: (url: string) => void }) => {
  ...
  if (isImage && msg.url) {
    return (
      <MediaImageWrapper>
        <img src={sanitizeMediaUrl(msg.url)} alt={msg.originalName} onClick={() => { const u = sanitizeMediaUrl(msg.url); if (u) openLightbox(u); }} onDoubleClick={(e) => e.preventDefault()} onContextMenu={(e) => e.preventDefault()} />
      </MediaImageWrapper>
    );
  }
